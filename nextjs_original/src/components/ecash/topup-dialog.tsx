...
import { requestEcashTopUp } from '@/services/server-actions';
...
export default function TopUpDialog({ isOpen, onOpenChange, balance }: TopUpDialogProps) {
  ...
  const onSubmit = async (values: TopUpFormValues) => {
    ...
    try {
      ...
      const result = await requestEcashTopUp({
        memberId: user.memberId || user.id,
        memberName: user.fullName || user.email || 'Unknown',
      ...
      if (result.success) {
        toast({ title: t('ecash.topup.requestSubmitted'), description: result.message });
        onOpenChange(false);
      } else {
        toast({ variant: 'destructive', title: t('ecash.topup.requestFailed'), description: result.message });
      }
    } catch (error: any) {
    ...
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md w-[90vw] rounded-lg">
        <DialogHeader>
          ...
          <DialogDescription>
             {t('ecash.topup.description', { balance: formatCurrency(balance) }).split(/(<strong>.*<\/strong>)/g).map((part, index) => {
                if (part.startsWith('<strong')) {
                    return <strong key={index}>{formatCurrency(balance)}</strong>;
                }
                return part;
             })}
          </DialogDescription>

...
export class EnhancedRuleEngine {
  ...
  /**
   * Evaluate enhanced formula with custom function support
   */
  private evaluateEnhancedFormula(
    ...
    try {
      ...
      let processedFormula = formula
        .replace(/PV/g, context.volumes.personal.toString())
        .replace(/GV/g, context.volumes.group.toString())
        .replace(/DR/g, context.team.directRecruits.toString())
        .replace(/TOTAL_DOWNLINE/g, context.team.totalDownline.toString());

      // Add custom variables from config
      Object.entries(config.globalOverrides || {}).forEach(([key, value]) => {

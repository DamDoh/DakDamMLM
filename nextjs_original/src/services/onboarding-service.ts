...
export async function initializeMemberOnboarding(memberId: string, templateId: string = 'new-member-onboarding'): Promise<MemberProgress | null> {
   try {
...
export async function getOnboardingTemplate(templateId: string): Promise<OnboardingTemplate | null> {
  const DEFAULT_ONBOARDING_TEMPLATE: OnboardingTemplate = {
      ...
      targetAudience: 'new_member' as const,
      version: '1.0',
      isActive: true,
      steps: [
        { id: 'welcome-video', title: 'Welcome to DakDam', description: 'Watch our welcome video', type: 'video' as const, isRequired: true, estimatedMinutes: 5, order: 1 },
        { id: 'company-overview', title: 'Company Overview', description: 'Learn about DakDam', type: 'document' as const, isRequired: true, estimatedMinutes: 15, order: 2 },
        { id: 'compensation-quiz', title: 'Compensation Plan Quiz', description: 'Test your understanding', type: 'quiz' as const, isRequired: true, estimatedMinutes: 10, order: 3 },
        { id: 'first-product-order', title: 'Place Your First Order', description: 'Experience our products', type: 'action' as const, isRequired: true, estimatedMinutes: 20, order: 4 },
        { id: 'setup-profile', title: 'Complete Your Profile', description: 'Set up your profile', type: 'action' as const, isRequired: true, estimatedMinutes: 10, order: 5 },
      ]
    };
...
export async function completeOnboardingStep(
   ...
   try {
     ...
     if (template) {
       const requiredSteps = template.steps.filter(step => step.isRequired).map(step => step.id);
       progress.isCompleted = requiredSteps.every(stepId => progress.completedSteps.includes(stepId));
     }
     ...
     // Note: revalidatePath removed due to pages directory compatibility

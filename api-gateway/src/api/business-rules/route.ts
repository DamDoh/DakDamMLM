...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (authenticatedRequest.status === 403) {
      logger.warn('Non-admin attempted to access business rules', {
        ip: request.headers.get('x-forwarded-for'),
        userAgent: request.headers.get('user-agent')
      }, request);
      return authenticatedRequest;
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (authenticatedRequest.status === 403) {
      logger.warn('Non-admin attempted to create business rule', {
        ip: request.headers.get('x-forwarded-for'),
        userAgent: request.headers.get('user-agent')
      }, request);
      return authenticatedRequest;
    ...
    await prisma.ruleValidationLog.create({
      data: {
        ...
        validatedBy: (authenticatedRequest as any).user?.id || 'system'
      }
    });

...
export class WalletServiceEnhanced {
  ...
  static async getOrCreateWallet(userId: string, companyId?: string): Promise<WalletData> {
    ...
    if (!wallet) {
      ...
      // Wallet creation logged via audit service
    }
  ...
  static async creditWallet(
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      // Financial transaction logged via audit service
  ...
  static async transfer(
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      if (!requiresEscrow) {
        ...
        // Transfer status logged via audit service
      } else {
        // Transfer pending approval logged via audit service
      }
...
// Backwards-compatible alias for existing imports expecting `WalletService`
export const WalletService = WalletServiceEnhanced;

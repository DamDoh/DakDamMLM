...
import { getHostingMailService } from '../../../../lib/hosting-mail-service';
...
class SimpleOTPService {
  ...
  constructor() {
    // Initialize hosting mail service
    this.mailService = getHostingMailService('generic', {
      fromEmail: process.env.SMTP_FROM_EMAIL || 'noreply@yourdomain.com',
      fromName: process.env.SMTP_FROM_NAME || 'MLM Platform',
  ...
  // Send OTP via email using hosting provider's mail system
  private async sendEmailOTP(email: string, code: string, purpose: string): Promise<boolean> {
    try {
      // Use the hosting mail service
      return await this.mailService.sendOTPEmail(email, code, purpose as 'verification' | 'password_reset' | 'login_2fa');
    } catch (error) {
  ...
  async generateOTP(request: SimpleOTPRequest): Promise<SimpleOTPResponse> {
    try {
      ...
      let sent = false;
      if (type === 'email') {
        sent = await this.sendEmailOTP(identifier, code, purpose);
      } else if (type === 'sms') {

...
class ComplianceService {
  ...
  async submitKYCDocument(
    ...
    try {
      ...
      return {
        ...
        extractedData: document.extractedData ? JSON.parse(String(document.extractedData)) : undefined,
      ...
      return {
        profile: {
          ...
          documents: kycProfile.documents.map(d => ({
            ...
            fileUrl: d.fileUrl,
            extractedData: d.extractedData ? JSON.parse(String(d.extractedData)) : undefined,
          ...
          checks: kycProfile.checks.map(c => ({
            ...
            provider: c.provider,
            result: JSON.parse(String(c.result)),
            score: c.score,
            checkedAt: c.createdAt || new Date(),
  ...
  private async performIdentityCheck(userId: string, kycProfile: any): Promise<KYCCheck> {
    ...
    return {
      ...
      result: JSON.parse(String(check.result)),
      score: check.score,

...
class ChurnPreventionService {
  ...
  async calculateChurnRisk(userId: string): Promise<ChurnRiskProfile> {
    try {
      ...
      logger.error('Churn risk calculation failed:', {
  ...
  async generateEngagementStrategy(userId: string, churnRisk: ChurnRiskProfile): Promise<EngagementStrategy> {
    try {
      ...
      const behaviorPatterns = await this._analyzeBehaviorPatternsInternal(memberData);
      ...
      logger.error('Engagement strategy generation failed:', {
        error: error instanceof Error ? error.message : String(error)
      });
      throw error;
    }
  ...
  async executeRetentionCampaign(campaignId: string): Promise<{
    ...
    try {
      ...
      logger.error('Retention campaign execution failed:', {
        error: error instanceof Error ? error.message : String(error)
      });
      throw error;
    }
  ...
  async analyzeBehaviorPatterns(userId: string): Promise<{
    ...
    try {
      ...
      logger.error('Behavior pattern analysis failed:', {
        error: error instanceof Error ? error.message : String(error)
      });
      throw error;
    }
  ...
  async getRetentionInsights(companyId?: string): Promise<{
    ...
    try {
      ...
      logger.error('Retention insights generation failed:', {
        error: error instanceof Error ? error.message : String(error)
      });
      throw error;
    }
  ...
  private async gatherMemberFeatures(userId: string): Promise<any> {
    ...
    // Get orders and commissions counts separately
    const [ordersCount, commissionsCount] = await Promise.all([
      prisma.order.count({
        where: { userId }
      }),
    ...
    return {
      ...
      totalCommissions: commissionsCount,
      recentOrders,
      recentCommissions,
  ...
  private async _analyzeBehaviorPatternsInternal(memberData: any): Promise<any> {
    // Analyze behavioral patterns
    return {
      loginFrequency: 'weekly',
      purchasePatterns: 'monthly',

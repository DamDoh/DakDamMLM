...
export class CommissionCalculationEngineEnhanced {
  ...
  static checkQualification(rank: string, volumeData: VolumeData): boolean {
    const requirements: Record<string, { personal: number; team: number }> = {
      'Member': { personal: 100, team: 200 },
      'Bronze': { personal: 500, team: 1000 },
      'Silver': { personal: 1000, team: 2500 },
      'Gold': { personal: 2500, team: 5000 },
      'Platinum': { personal: 5000, team: 10000 },
      'Diamond': { personal: 10000, team: 25000 }
    };
    ...
    const minLegRequirement = req.team * 0.3; // At least 30% on weaker leg

    return volumeData.personalVolume >= req.personal && 
           volumeData.totalTeamVolume >= req.team &&
           minLegVolume >= minLegRequirement;
  }
  ...
  static async calculateRankAdvancementBonus(
    ...
    // Note: JSON path filtering removed due to Prisma limitations
      ...
      if (calc.commissionAmount > 0 && calc.qualificationMet) {
        try {
          ...
          await prisma.$transaction(async (tx) => {
            ...
            console.log(`Processed commission of ${calc.commissionAmount} for member ${calc.memberId}`);
          }, {
          ...
          // ENHANCEMENT: Log to audit system - keep JSON-safe payload

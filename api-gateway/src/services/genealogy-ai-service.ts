...
class GenealogyAIService {
  ...
  private async getNetworkStructure(companyId?: string): Promise<any> {
    ...
    const networkStats = await prisma.user.aggregate({
      where: {
        active: true,
        accountType: { in: ['Distributor', 'Stockist'] },
        ...(companyId && { companyId })
    ...
    return {
      totalMembers,
      totalRelationships: 0, // TODO: Calculate from separate query if needed
      averageTeamSize: 0 // TODO: Calculate from separate query if needed
    };
  }
  ...
  private calculateActivityMatch(member: any, sponsor: any): number {
    ...
    const sponsorActivity = sponsor.lastActivityDate
      ? Math.floor((Date.now() - sponsor.lastActivity.getTime()) / (1000 * 60 * 60 * 24))
      : 999;

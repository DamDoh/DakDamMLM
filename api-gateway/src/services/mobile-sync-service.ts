...
class MobileSyncService {
  ...
  async getMobileUserData(
    ...
    try {
      ...
      logger.error('Failed to get mobile user data:', { error: error instanceof Error ? error.message : String(error) });
      throw error;
    }
  }
  ...
  async processSyncOperations(
    ...
    try {
      ...
      logger.error('Sync operation processing failed:', { error: error instanceof Error ? error.message : String(error) });
  ...
  async getIncrementalChanges(
    ...
    try {
      ...
      logger.error('Failed to get incremental changes:', { error: error instanceof Error ? error.message : String(error) });
      throw error;
    }
  }
  ...
  async queueOfflineOperation(
    ...
    try {
      ...
      logger.error('Failed to queue offline operation:', { error: error instanceof Error ? error.message : String(error) });
      throw error;
    }
  }
  ...
  async processQueuedOperations(
    ...
    try {
      ...
      for (const operation of operations) {
        try {
          // Convert database operation to SyncOperation format
          ...
          logger.error(`Failed to process operation ${operation.id}:`, { error: error instanceof Error ? error.message : String(error) });
          failed++;
      ...
      logger.error('Failed to process queued operations:', { error: error instanceof Error ? error.message : String(error) });
      throw error;
    }
  }
  ...
  async getMobileDashboard(
    ...
    try {
      ...
      logger.error('Failed to get mobile dashboard:', { error: error instanceof Error ? error.message : String(error) });
      throw error;
    }
  }
  ...
  async validateAppVersion(
    ...
    try {
      ...
      const criticalUpdate: boolean = criticalVersion ? this.compareVersions(appVersion, criticalVersion) < 0 : false;
      ...
      logger.error('App version validation failed:', { error: error instanceof Error ? error.message : String(error) });
      return {
        compatible: false,
        criticalUpdate: true,
        features: []
  ...
  private async getMobileGenealogyData(userId: string): Promise<any> {
    ...
    return {
      ...
      directDownline,
      totalTeamSize: (() => {
        if (!user.teamSize) return 0;
        if (typeof user.teamSize === 'string') {
          try {
            const parsed = JSON.parse(user.teamSize) as any;
            return parsed?.total || 0;
          } catch {
            return 0;
          }
        }
        if (typeof user.teamSize === 'object' && user.teamSize !== null) {
          return (user.teamSize as any)?.total || 0;
        }
        return 0;
      })()
    };

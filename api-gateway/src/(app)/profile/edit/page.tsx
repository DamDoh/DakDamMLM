...
export default function EditProfilePage() {
  ...
  const onSubmit = async (values: ProfileFormValues) => {
    ...
    try {
      ...
      const conflicts = await checkProfileConflicts(user.id, { phoneNumber: values.phoneNumber, email: values.email || undefined });

      if (conflicts.phoneConflict) {
        toast({
          variant: 'destructive',
          title: 'Phone Number Conflict',
          description: 'This phone number is already registered to another user. Please use a different phone number.',
        });
        setIsLoading(false);
        return;
      }
      ...
      if (conflicts.emailConflict) {
        toast({
          ...
          description: 'This email address is already registered to another user. Please use a different email address.',
        });
        setIsLoading(false);
        return;
      }
      ...
      if (success) {
        ...
        toast({
          variant: 'destructive',
          title: 'Update Failed',
          description: 'Failed to save profile changes. Please check your information and try again.',
        });
      }
    } catch (error) {

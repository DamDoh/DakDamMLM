...
import { Calendar, Hash, Star, Users, Store, UserCheck, ArrowUpCircle, Upload, Edit, ShieldCheck, MapPin, PlusCircle, KeyRound, Trash2 } from 'lucide-react';
            ...
            <Card className="max-w-2xl mx-auto">
                <CardHeader>
                   <div className="flex items-center justify-between">
                     <div>
                       <CardTitle className="flex items-center gap-2"><MapPin/> My Addresses</CardTitle>
                       <CardDescription>Manage your saved shipping and pickup locations.</CardDescription>
                     </div>
                      <Button variant="outline" icon={PlusCircle} onClick={handleOpenAddAddress}>Add New</Button>
                   </div>
                </CardHeader>
                <CardContent>
                    <div className="space-y-4">
                        {addresses.map((addr) => (
                            <div key={addr.id} className="flex flex-col sm:flex-row items-start justify-between p-4 border rounded-lg gap-4">
                                ...
                                <div className="flex items-center gap-2 self-start sm:self-center">
                                    {!addr.isDefault && (
                                        <Button variant="ghost" size="sm" onClick={() => handleSetDefaultAddress(addr.id)}>Set as Default</Button>
                                    )}
                                    <Button variant="outline" size="sm" icon={Edit} onClick={() => handleOpenEditAddress(addr)}>Edit</Button>

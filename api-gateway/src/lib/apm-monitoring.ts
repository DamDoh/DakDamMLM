...
import { BatchSpanProcessor, ConsoleSpanExporter, SamplingResult } from '@opentelemetry/sdk-trace-base';
...
class APMMonitoringService {
  ...
  private initializeAPM(): void {
    // Dynamically import Resource to avoid type-only import issue
    const { Resource } = require('@opentelemetry/resources');
    const resource = new Resource({
      [SemanticResourceAttributes.SERVICE_NAME]: this.config.serviceName,
      [SemanticResourceAttributes.SERVICE_VERSION]: this.config.serviceVersion,
      [SemanticResourceAttributes.SERVICE_NAMESPACE]: 'mlm-platform'
    });
    ...
    this.sdk = new NodeSDK({
      ...
      sampler: {
        shouldSample: (context, traceId, spanName, spanKind, attributes, links): SamplingResult => {
          ...
          // Decision: 0 = DROP, 1 = RECORD_ONLY, 2 = RECORD_AND_SAMPLE
          return {
            decision: shouldSample ? 2 : 0
          };
        }
      }
    });

...
// Core health monitoring logic and service checks

import { prisma } from '@/lib/database';
import { SystemHealthReport, ServiceHealth, BusinessMetrics } from './health-types';
...
export class SystemHealthMonitor {
  ...
  async generateHealthReport(): Promise<SystemHealthReport> {
    ...
    try {
      ...
      // Return a degraded report instead of throwing to keep monitoring running
      return this.createFallbackHealthReport(error);
    }
  }
  ...
  private async getBusinessMetrics(): Promise<BusinessMetrics> {
    try {
      ...
      // Return default values instead of throwing to prevent health check failures
      return {
        totalMembers: 0,
        activeMembers: 0,
        totalCommissions: 0,
  ...
  private createFallbackHealthReport(error: any): SystemHealthReport {
    return {
      timestamp: new Date().toISOString(),
      overall: 'unhealthy',
      uptime: process.uptime(),
      version: process.env.npm_package_version || '1.0.0',
      services: {
        web: this.createUnhealthyService('Health check failed'),
        database: this.createUnhealthyService('Health check failed'),
        cache: this.createUnhealthyService('Health check failed'),
        messageQueue: this.createUnhealthyService('Health check failed'),
        analytics: this.createUnhealthyService('Health check failed'),
        genealogy: this.createUnhealthyService('Health check failed'),
        commission: this.createUnhealthyService('Health check failed'),
        notification: this.createUnhealthyService('Health check failed'),
      },
      performance: {
  ...
  private async getConnectionPoolInfo(): Promise<any> {
    try {
      // Return basic connection info since we don't have direct access to pool details
      return { status: 'connected', type: 'prisma' };
    } catch (error) {

...
class AdvancedI18nService {
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      logger.error('Failed to initialize i18n service:', error instanceof Error ? error : { error: String(error) });
      throw error;
    }
  }
  ...
  async changeLanguage(languageCode: string): Promise<void> {
    ...
    try {
      ...
      logger.error(`Failed to change language to ${languageCode}:`, error instanceof Error ? error : { error: String(error) });
      throw error;
    }
  }
  ...
  t(key: string, options: TranslationContext & any = {}): string {
    try {
      ...
      logger.warn(`Translation failed for key: ${key}`, { error: error instanceof Error ? error.message : String(error) });
  ...
  async loadTranslations(languageCode: string, namespace: string = 'common'): Promise<void> {
    try {
      ...
      logger.error(`Failed to load translations for ${languageCode}:${namespace}:`, error instanceof Error ? error : { error: String(error) });
      throw error;
    }
  }
  ...
  async addTranslations(languageCode: string, namespace: string, translations: Record<string, any>): Promise<void> {
    try {
      ...
      logger.error(`Failed to add translations for ${languageCode}:${namespace}:`, error instanceof Error ? error : { error: String(error) });
      throw error;
    }
  }
  ...
  async getLocalizedContent(contentId: string, languageCode?: string): Promise<LocalizedContent | null> {
    ...
    try {
      ...
      logger.error(`Failed to get localized content ${contentId}:`, error instanceof Error ? error : { error: String(error) });
      return null;
    }
  }
  ...
  async exportTranslations(languageCode: string, namespaces: string[] = ['common']): Promise<Record<string, any>> {
    ...
    for (const ns of namespaces) {
      try {
        ...
        logger.warn(`Failed to export namespace ${ns} for ${languageCode}:`, { error: error instanceof Error ? error.message : String(error) });
      }
    }
  ...
  async importTranslations(languageCode: string, translations: Record<string, any>): Promise<void> {
    try {
      ...
      logger.error(`Failed to import translations for ${languageCode}:`, error instanceof Error ? error : { error: String(error) });
      throw error;
    }
  }
      ...
      for (const [code, config] of this.languages) {
        ...
        if (resources) {
          ...
          for (const key of baseKeys) {
            if (resources[key] && resources[key] !== key) {
              translatedKeys++;
            } else {
  ...
  async shutdown(): Promise<void> {
    try {
      ...
      logger.error('Error shutting down i18n service:', error instanceof Error ? error : { error: String(error) });
    }
  }
}
...
export const useTranslation = (): {
  ...
  formatCurrency: (value: number, currency?: string, options?: any) => string;
  formatDate: (date: string | Date, options?: any) => string;
  getPluralForm: (count: number, key: string, options?: any) => string;
  accessibility: any;
  seo: (pageKey: string) => any;
} => {
  const { t: i18nT, i18n: i18nInstance } = useI18nextTranslation('common');

...
class MultiTenantResourceManager {
  ...
  async checkResourceUsage(
    ...
    switch (resourceType) {
      case 'users':
        limit = (resource as any).maxUsers || 0;
        usageField = 'currentUsers';
        break;
      case 'storage':
        limit = (resource as any).maxStorageGB || 0;
        usageField = 'currentStorageGB';
        break;
      case 'api':
        // Check both hourly and daily limits
        const hourlyLimit = (resource as any).maxRequestsPerHour || 0;
        const dailyLimit = (resource as any).maxRequestsPerDay || 0;
      ...
      case 'database':
        limit = (resource as any).maxConnections || 0;
        usageField = 'currentConnections';
        break;
      case 'email':
        limit = (resource as any).maxEmailsPerDay || 0;
        usageField = 'currentEmailsToday';
        break;
      case 'genealogy':
        limit = (resource as any).maxDownlineSize || 0;
  ...
  async recordUsage(
    ...
    try {
      ...
      // Update APM metrics (resource_usage is not a valid business metric)
      // Note: Valid business metrics are: ordersCreated, commissionsCalculated, usersRegistered, churnPreventionActions
      // apmMonitoring.recordBusinessMetric('resource_usage', amount, {
      //   tenant_id: tenantId,
  ...
  async updateTenantQuota(
    ...
    try {
      ...
      logger.error('Failed to update tenant quota:', {
        error: error instanceof Error ? error.message : String(error)
      });
      throw error;
    }
  ...
  async applyScalingRecommendation(
    ...
    try {
      ...
      logger.error('Failed to apply scaling recommendation:', {
        error: error instanceof Error ? error.message : String(error)
      });
      throw error;
    }
  ...
  async getSystemResourceSummary(): Promise<{
    ...
    try {
      ...
      logger.error('Failed to get system resource summary:', {
        error: error instanceof Error ? error.message : String(error)
      });
      throw error;
    }
  ...
  private async monitorAllTenants(): Promise<void> {
    try {
      ...
      logger.error('Failed to monitor tenants:', {
        error: error instanceof Error ? error.message : String(error)
      });
    }
  }
  ...
  private async monitorTenant(tenantId: string): Promise<void> {
    try {
      ...
      logger.error(`Failed to monitor tenant ${tenantId}:`, {
        error: error instanceof Error ? error.message : String(error)
      });
    }
  }
  ...
  private async processMonthlyBilling(): Promise<void> {
    try {
      ...
      logger.error('Monthly billing processing failed:', {
        error: error instanceof Error ? error.message : String(error)
      });
    }
  }
  ...
  private async processTenantBilling(tenantId: string): Promise<void> {
    try {
      ...
      logger.error(`Failed to process billing for tenant ${tenantId}:`, {
        error: error instanceof Error ? error.message : String(error)
      });
    }
  }

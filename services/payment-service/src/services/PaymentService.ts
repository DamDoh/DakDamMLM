...
export class PaymentService {
  async processPayment(paymentData: ProcessPaymentData) {
    ...
    try {
      ...
      await cacheService.setCachedTransaction(transactionId, processedTransaction);

      // Invalidate user transactions cache
      await cacheService.invalidateUserTransactionsCache(userId);
      ...
      await (db as any).paymentTransaction.updateMany({
        where: { transactionId },
        data: {
          status: 'FAILED',
          failedAt: new Date(),
  ...
  async processRefund(transactionId: string, amount: number, reason: string) {
      const originalTransaction = await (db as any).paymentTransaction.findUnique({
    ...
    try {
      ...
      // Invalidate caches
      await cacheService.invalidateTransactionCache(transactionId);
      await cacheService.invalidateUserTransactionsCache(originalTransaction.userId);
  ...
  async getTransactionById(transactionId: string) {
    // Try cache first
    let transaction = await cacheService.getCachedTransaction(transactionId);
    if (transaction) {
      return transaction;
    }

    // Fetch from database
    transaction = await (db as any).paymentTransaction.findUnique({
      where: { id: transactionId },
      include: {
        user: {
          select: {
    ...
    if (transaction) {
      ...
      await cacheService.setCachedTransaction(transactionId, transaction);
    }

    return transaction;
  }

  async getUserTransactions(userId: string, page: number = 1, limit: number = 10, type?: string) {
    const skip = (page - 1) * limit;
  ...
  async getPaymentAnalytics(query: AnalyticsQuery) {
    const { startDate, endDate } = this.getDateRange(query);

    const [transactionStats, methodStats, providerStats] = await Promise.all([
      // Transaction status distribution
      (db as any).paymentTransaction.groupBy({
        by: ['status'],
        where: {
          createdAt: {
            gte: startDate,
    ...
    const statusDistribution = transactionStats.reduce((acc: Record<string, number>, stat: any) => {
      acc[stat.status] = stat._count;
      return acc;
    }, {} as Record<string, number>);

    const paymentMethodDistribution = methodStats.reduce((acc: Record<string, { count: number; volume: number }>, stat: any) => {
      acc[stat.method] = {
        count: stat._count,
    ...
    const revenueByProvider = providerStats.reduce((acc: Record<string, number>, stat: any) => {
      acc[stat.provider] = stat._sum.netAmount || 0;
      return acc;
    }, {} as Record<string, number>);

    // Calculate success rate
    const totalTransactions = transactionStats.reduce((sum: number, stat: any) => sum + stat._count, 0);
    ...
    return {
      ...
      endDate,
      totalTransactions,
      totalVolume: methodStats.reduce((sum: number, stat: any) => sum + (stat._sum.amount || 0), 0),
      averageTransactionValue: totalTransactions > 0
        ? methodStats.reduce((sum: number, stat: any) => sum + (stat._sum.amount || 0), 0) / totalTransactions
        : 0,
      successRate,
      statusDistribution,
  ...
  async getRevenueAnalytics(query: AnalyticsQuery) {
    ...
    // Top paying users
    const topPayingUsers = await (db as any).paymentTransaction.groupBy({
      by: ['userId'],
      where: {
        createdAt: {
          gte: startDate,
    ...
    return {
      ...
      endDate,
      revenueByPeriod,
      topPayingUsers: topPayingUsers.map((user: any) => ({
        userId: user.userId,
  ...
  private async processWithProvider(transaction: any, paymentData: any) {
    ...
    try {
      ...
      if (isSuccess) {
        return await (db as any).paymentTransaction.update({
      ...
      throw new Error(`Payment provider error: ${(error as Error).message}`);
    }
  }

  private async processRefundWithProvider(refund: any, originalTransaction: any) {
    // Simulate refund processing
    try {
      await new Promise(resolve => setTimeout(resolve, 50));

      return await (db as any).refund.update({
        where: { id: refund.id },
        data: {
          status: 'COMPLETED',

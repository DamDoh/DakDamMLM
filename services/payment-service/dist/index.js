...
const payoutRoutes_1 = require("./routes/payoutRoutes");
...
app.use((req, res, next) => {
    ...
    const sanitizeString = (str) => str.replace(/<script>/gi, '').replace(/<\/script>/gi, '');
...
const limiter = (0, express_rate_limit_1.default)({
    ...
    message: (req) => ({
        success: false,
        error: req.t ? req.t('errors.too_many_requests') : 'Too many requests from this IP, please try again later.',
...
app.use((req, res, next) => {
    ...
    res.on('finish', () => {
        ...
        (0, logger_1.logPerformanceMetric)('http_request', duration, {
            method: req.method,
            path: req.path,
...
app.use('/api/payouts', payoutRoutes_1.payoutRoutes);
...
app.post('/webhooks/stripe', express_1.default.raw({ type: 'application/json' }), (req, res) => {
    ...
    logger_1.logger.info('Stripe webhook received', { type: 'webhook_stripe' });
    res.json({ received: true });
});
app.post('/webhooks/paypal', (req, res) => {
    // Handle PayPal webhooks
    logger_1.logger.info('PayPal webhook received', { type: 'webhook_paypal' });
    res.json({ received: true });
});
// 404 handler
app.use((req, res) => {
    ...
    res.status(404).json({
        success: false,
        error: req.t ? req.t('errors.route_not_found') : 'Route not found',
        path: req.path,
        method: req.method,
    });
});
...
app.use((err, req, res, next) => {
    ...
    const errorResponse = {
        success: false,
        error: isDevelopment ? err.message : (req.t ? req.t('errors.internal_server_error') : 'Internal server error'),
        requestId,
...
const server = app.listen(PORT, async () => {
    logger_1.logger.info('Payment service started', {
        port: PORT,

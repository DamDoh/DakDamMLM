...
class CommissionService {
    ...
    async calculateBinaryCommissions(order, user, genealogy, rules) {
        ...
        const binaryCommissions = await this.calculateBinaryFromGenealogy(user.id, genealogy, order, rules);
        commissions.push(...binaryCommissions);
        return commissions;
    }
    async calculateBinaryFromGenealogy(userId, genealogy, order, rules) {
        const commissions = [];
            ...
            if (sponsor.children && sponsor.children.left && sponsor.children.right) {
                ...
                if (applicableRule) {
                    commissions.push({
                        ...
                        type: 'BINARY',
                        level: 1,
                        amount: (order.totalPV * applicableRule.percentage) / 100,
    ...
    async healthCheck() {
        try {
            await database_1.commissionDatabaseConnection.healthCheck();

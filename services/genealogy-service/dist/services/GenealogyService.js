...
class GenealogyService {
    ...
    async determinePosition(parentId, childId) {
        ...
        if (parent?.children?.left === childId)
            return 'left';
        if (parent?.children?.right === childId)
            return 'right';
        return null;
    }
    ...
    async moveDownline(userId, newParentId, position) {
        try {
            ...
            if ((position === 'left' && newParent.children?.left) ||
                (position === 'right' && newParent.children?.right)) {
    ...
    async getDownlineWithPagination(userId, page = 1, limit = 50, includeInactive = false, sortBy = 'joinDate', sortOrder = 'desc') {
        try {
            const user = await this.repository.findUserById(userId);
            if (!user) {
                throw utils_1.ServiceErrorHandler.createError('NOT_FOUND', `User with ID ${userId} not found`);
    ...
    async buildBinaryTree(user, currentLevel, maxDepth) {
        ...
        if (currentLevel < maxDepth && user.children) {
            // Only include direct children (left and right)
            if (user.children.left) {
                const leftChild = await this.repository.findUserWithChildren(user.children.left);
                if (leftChild) {
            ...
            if (user.children.right) {
                const rightChild = await this.repository.findUserWithChildren(user.children.right);
                if (rightChild) {

...
class GenealogyController {
    constructor() {
        ...
        this.getDownlinePaginated = async (req, res) => {
            try {
                const { userId } = req.params;
                const { page = 1, limit = 50, includeInactive = false, sortBy = 'createdAt', sortOrder = 'desc' } = req.query;
                // Users can only view their own downline or admin can view any
                if (!req.user?.isAdmin && req.user?.id !== userId) {
                    return res.status(403).json(this.createResponse(null, false, 'Access denied'));
                }
                const validSortBy = ['createdAt', 'pv', 'rank'].includes(sortBy)
                    ? sortBy
                    : 'createdAt';
                const result = await this.genealogyService.getDownlineWithPagination(userId, parseInt(page), parseInt(limit), includeInactive === 'true', validSortBy, sortOrder);

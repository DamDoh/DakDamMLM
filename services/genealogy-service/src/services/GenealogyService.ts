import { GenealogyRepository } from '../repositories/GenealogyRepository';
import { GenealogyNode, GenealogyStats, PlacementInfo, MoveDownlineResponse } from '../models/GenealogyTypes';
import { ServiceErrorHandler } from '../../../../shared/utils';
import { PrismaClient } from '@prisma/client';

const db = new PrismaClient();

export class GenealogyService {
  private repository: GenealogyRepository;

  constructor() {
    this.repository = new GenealogyRepository();
  }
  async getGenealogyTree(userId: string, maxDepth: number = 3): Promise<GenealogyNode | null> {
    try {
      const rootUser = await this.repository.findUserWithChildren(userId);

      if (!rootUser) {
        throw ServiceErrorHandler.createError('NOT_FOUND', `User with ID ${userId} not found`);
      }
  ...
  private async buildTree(user: any, currentLevel: number, maxDepth: number): Promise<GenealogyNode> {
  ...
  async getDownline(userId: string, maxLevel?: number, includeStats: boolean = false, includeInactive: boolean = false): Promise<any> {
    try {
  ...
  private async getAllDownline(userId: string, maxLevel?: number, includeInactive: boolean = false): Promise<any[]> {
    const downline: any[] = [];
    const queue: Array<{ id: string; level: number; position: 'left' | 'right' | null }> = [
      { id: userId, level: 0, position: null }
    ];

    while (queue.length > 0) {
      const current = queue.shift()!;
      if (maxLevel && current.level >= maxLevel) continue;

      const children = await this.repository.findDownlineBySponsor(current.id);

      // Filter inactive users if not including them
      const filteredChildren = includeInactive ? children : children.filter((child: any) => child.active);
  ...
  private async determinePosition(parentId: string, childId: string): Promise<'left' | 'right' | null> {
    const parent = await this.repository.findUserById(parentId);
    if (!parent?.children) return null;

    const children = parent.children as { left?: string | null; right?: string | null };
    if (children.left === childId) return 'left';
    if (children.right === childId) return 'right';
    return null;
  }

  async getUpline(userId: string): Promise<any[]> {
    try {
      // Validate user exists
      const user = await this.repository.findUserById(userId);
      if (!user) {
  ...
  async moveDownline(userId: string, newParentId: string, position: 'left' | 'right'): Promise<MoveDownlineResponse> {
    try {
      ...
      if (!isValidMove) {
        throw ServiceErrorHandler.createError('VALIDATION_ERROR', 'Invalid placement move');
      }

      // Validate that the new parent exists
      const newParent = await this.repository.findUserById(newParentId);
      if (!newParent) {
        throw ServiceErrorHandler.createError('NOT_FOUND', `New parent with ID ${newParentId} not found`);
      }

      // Check if position is available
      const newParentChildren = newParent.children as { left?: string | null; right?: string | null } | null;
      if ((position === 'left' && newParentChildren?.left) ||
          (position === 'right' && newParentChildren?.right)) {
        throw ServiceErrorHandler.createError('VALIDATION_ERROR', `Position ${position} is already occupied`);
      }

      // Check for cycle: ensure newParentId is not in user's downline
      const downline = await this.getAllDownline(userId);
      if (downline.some(member => member.id === newParentId)) {
        throw ServiceErrorHandler.createError('VALIDATION_ERROR', 'Cannot move user under their own downline - would create a cycle');
      }
      ...
      if (oldParentId) {
        ...
        if (currentParent && currentParent.children) {
          const currentChildren = currentParent.children as { left?: string | null; right?: string | null };
          const updatedChildren = { ...currentChildren };
          if (updatedChildren.left === userId) updatedChildren.left = null;
          if (updatedChildren.right === userId) updatedChildren.right = null;
  ...
  async getGenealogyStats(userId: string): Promise<GenealogyStats> {
    try {
      // Validate user exists
      const user = await this.repository.findUserById(userId);
      if (!user) {
  ...
  async getPlacementInfo(userId: string): Promise<PlacementInfo> {
    try {
      ...
      return {
        ...
        position: user.position as 'left' | 'right' | null,
        children: user.children as { left: string | null; right: string | null },
      };
    } catch (error) {
  ...
  private async calculateDownlineStats(userId: string, downline: any[]): Promise<any> {
  ...
  async getDownlineWithPagination(
    userId: string,
    page: number = 1,
    limit: number = 50,
    includeInactive: boolean = false,
    sortBy: 'createdAt' | 'pv' | 'rank' = 'createdAt',
  ...
  private sortDownline(downline: any[], sortBy: string, sortOrder: 'asc' | 'desc') {
  ...
  async getBinaryTree(userId: string, maxDepth: number = 3): Promise<GenealogyNode | null> {
    try {
      const rootUser = await this.repository.findUserWithChildren(userId);
  ...
  private async buildBinaryTree(user: any, currentLevel: number, maxDepth: number): Promise<GenealogyNode> {
    const node: GenealogyNode = {
      id: user.id,
      memberId: user.memberId || '',
      fullName: user.fullName,
  ...
  async getGenealogyMetrics(userId: string): Promise<any> {
    try {
      const user = await this.repository.findUserById(userId);
      if (!user) {
        throw ServiceErrorHandler.createError('NOT_FOUND', `User with ID ${userId} not found`);
  ...
  private calculateGrowthVelocity(downline: any[]): number {
  ...
  private calculateRetentionRate(downline: any[]): number {
    if (downline.length === 0) return 0;
  ...
  async bulkUpdateGenealogy(operations: Array<{

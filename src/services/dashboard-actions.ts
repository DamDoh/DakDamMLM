...
import * as complianceServiceOriginal from '@/../nextjs_original/src/services/compliance-service';
...
export async function getActiveComplianceDocuments(type?: any, companyId?: string): Promise<any[]> {
  try {
    ...
    // @ts-ignore - using original helper for now
    return await complianceServiceOriginal.getActiveComplianceDocuments(type, companyId);
  } catch (error) {
    const errorMessage = error instanceof Error ? error.message : String(error);

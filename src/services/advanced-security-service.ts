...
class AdvancedSecurityService {
    ...
    try {
      ...
      const deviceFingerprint = await prisma.deviceFingerprint.upsert({
        ...
        create: {
          ...
          userAgent,
          ipAddress,
          // Store raw JSON objects; let Prisma handle JSON serialization
      ...
      return {
        ...
        // Stored as JSON in Prisma; return as-is (or cast) instead of parsing strings
        location: deviceFingerprint.location as any,
        deviceInfo: deviceFingerprint.deviceInfo as any,
      ...
      logger.error('Device fingerprint registration failed:', { error });
      throw error;
    }
  }
  ...
  private async calculateDeviceTrustScore(
    ...
    try {
      ...
      const consistentPlatform = recentDevices.every(d => {
        ...
        return info && info.platform === deviceInfo.platform;
      });
  ...
  async registerBiometricCredential(
    ...
    try {
      ...
      logger.error('Biometric credential registration failed:', { error });
      throw error;
    }
  }
  ...
  async createSecuritySession(
    ...
    try {
      ...
      return {
        ...
        // Stored as JSON in DB; cast to string[] for the session contract
        authenticationMethods: (session.authenticationMethods as any) as string[],
      ...
      logger.error('Security session creation failed:', { error });
      throw error;
    }
  }
  ...
  private async getBehavioralPattern(userId: string): Promise<BehavioralPattern | null> {
    try {
      ...
      logger.error('Behavioral pattern retrieval failed:', { error });
      return null;
    }
  }

...
import { Download, FileText, Copy, RefreshCw, BookOpen, Code, BarChart3, Loader2 } from 'lucide-react';
...
export function RuleDocumentation({ rules, templates }: RuleDocumentationProps) {
  ...
  const generateRuleDocumentation = useCallback(async (rulesData: BusinessRule[], templatesData: RuleTemplate[]): Promise<string> => {
    const sections = [];

    // Title and Overview
    sections.push('# Business Rules Documentation\n');
    sections.push(`Generated on: ${new Date().toLocaleString()}\n`);
    sections.push(`Total Rules: ${rulesData.length}\n`);
    sections.push(`Active Rules: ${rulesData.filter(r => r.isActive).length}\n`);
    sections.push(`Available Templates: ${templatesData.length}\n\n`);
    ...
    const categories = rulesData.reduce((acc, rule) => {
    ...
    const types = rulesData.reduce((acc, rule) => {
    ...
    categoriesOrder.forEach(category => {
      const categoryRules = rulesData.filter(r => r.category === category && r.isActive);
      ...
      categoryRules.forEach(rule => {
        ...
        sections.push(`**Applicable To**: ${Array.isArray(rule.applicableTo) ? rule.applicableTo.join(', ') : 'N/A'}\n\n`);
        ...
        if (rule.conditions && Array.isArray(rule.conditions) && rule.conditions.length > 0) {
          ...
          rule.conditions.forEach((condition: any) => {
            sections.push(`- ${condition.type || 'condition'} ${condition.operator || '='} ${JSON.stringify(condition.value || condition)}\n`);
          });
          sections.push('\n');
        }

        // Calculation
        if (rule.calculation && typeof rule.calculation === 'object') {
          sections.push('**Calculation**:\n');
          const calc = rule.calculation as any;
          if (calc.type) {
            sections.push(`- Type: ${calc.type}\n`);
          }
          if (calc.percentage !== undefined) {
            sections.push(`- Percentage: ${calc.percentage}%\n`);
          }
          if (calc.baseValue !== undefined) {
            sections.push(`- Base Value: ${calc.baseValue}\n`);
          }
          if (calc.tiers && Array.isArray(calc.tiers) && calc.tiers.length > 0) {
            sections.push('- Tiers:\n');
            calc.tiers.forEach((tier: any) => {
    ...
    if (templatesData.length > 0) {
      ...
      templatesData.forEach(template => {
        ...
        sections.push(`**Applicable Markets**: ${Array.isArray(template.applicableMarkets) ? template.applicableMarkets.join(', ') : 'N/A'}\n\n`);

        if (template.description) {
          sections.push(`**Description**: ${template.description}\n\n`);
        }

        sections.push(`**Rules Included**: ${Array.isArray(template.rules) ? template.rules.length : 0}\n\n`);

        // Template rules summary
        if (Array.isArray(template.rules)) {
          const templateCategories = template.rules.reduce((acc: Record<string, number>, rule: any) => {
          acc[rule.category] = (acc[rule.category] || 0) + 1;
          return acc;
        }, {} as Record<string, number>);
    ...
    allRuleTypes.forEach(type => {
      const count = rulesData.filter(r => r.type === type).length;
      if (count > 0) {
    ...
    sections.push('- `GET /api/business-rules` - List all rules\n');
    sections.push('- `POST /api/business-rules` - Create new rule\n');
    sections.push('- `PUT /api/business-rules/:id` - Update rule\n');
    sections.push('- `DELETE /api/business-rules/:id` - Delete rule\n');
    sections.push('- `POST /api/business-rules/simulate` - Simulate rule execution\n');
    sections.push('- `GET /api/business-rules/conflicts` - Check for conflicts\n\n');
  ...
  const generateDocumentation = useCallback(async () => {
    ...
    try {
      ...
      toast({
        title: t('businessRules.documentationGenerated'),
        description: t('businessRules.documentationGeneratedDesc'),
      });
    } catch (error) {
      ...
      toast({
        ...
        description: error instanceof Error ? error.message : 'Failed to generate rule documentation.',
      });
    } finally {
      setLoading(false);
    }
  }, [rules, templates, generateRuleDocumentation, toast]);

  useEffect(() => {
    // Generate documentation on initial load when rules are available
    // Only run once on mount or when rules/templates first become available
    if ((rules.length > 0 || templates.length > 0) && !documentation) {
      generateDocumentation();
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []); // Only run on mount to avoid regenerating on every rules/templates change
  ...
  return (
    <div className="space-y-6">
          ...
          <div className="flex items-center justify-between">
            <div>
              <CardTitle className="flex items-center gap-2">
                ...
                {t('businessRules.ruleDocumentationGenerator')}
              </CardTitle>
              <CardDescription>
                {t('businessRules.generateComprehensiveDocs')}
              </CardDescription>
            </div>
              ...
              {documentation && (
                <>
                  <Button onClick={downloadDocumentation} variant="outline">
                    ...
                    {t('businessRules.download')}
                  </Button>
                  <Button onClick={copyToClipboard} variant="outline">
                    <Copy className="h-4 w-4 mr-2" />
                    {t('businessRules.copy')}
                  </Button>
                </>
              )}
            </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <Card>
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-muted-foreground">{t('businessRules.totalRules')}</p>
        ...
        <Card>
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-muted-foreground">{t('businessRules.activeRules')}</p>
        ...
        <Card>
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-muted-foreground">{t('businessRules.categories')}</p>
        ...
        <Card>
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-muted-foreground">{t('businessRules.templates')}</p>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        <TabsList>
          <TabsTrigger value="overview">{t('businessRules.overview')}</TabsTrigger>
          <TabsTrigger value="categories">{t('businessRules.categories')}</TabsTrigger>
          <TabsTrigger value="templates">{t('businessRules.templates')}</TabsTrigger>
          <TabsTrigger value="full-docs">{t('businessRules.fullDocumentation')}</TabsTrigger>
        </TabsList>

        <TabsContent value="overview" className="space-y-4">
          <Card>
            <CardHeader>
              <CardTitle>{t('businessRules.rulesByCategory')}</CardTitle>
            </CardHeader>
            <CardContent>
              <Table>
                <TableHeader>
                  <TableRow>
                    ...
                    <TableHead>{t('businessRules.activeRules')}</TableHead>
                    <TableHead>{t('businessRules.totalRulesCol')}</TableHead>
                    <TableHead>{t('businessRules.percentageCol')}</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>

...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    // Get all active companies for registration
    // Newly registered companies are set to isActive: true so they appear immediately
    let companies = await prisma.company.findMany({
      where: { 
        isActive: true // Show all active companies (verified or pending verification)
      },
      select: {
        id: true,
        name: true,
    ...
    // Ensure "DakDam MLM" always exists and is in the list
    const dakdamCompany = companies.find(c => c.name === 'DakDam MLM');
    if (!dakdamCompany) {
      // Create or update DakDam MLM company
      ...
      // Add to companies array at the beginning (priority)
      companies = [
        {
          id: defaultCompany.id,
          name: defaultCompany.name,
          domain: defaultCompany.domain,
          ...
          timezone: defaultCompany.timezone,
          isActive: defaultCompany.isActive,
          isVerified: defaultCompany.isVerified,
          allowEmailLogin: defaultCompany.allowEmailLogin,
          allowPhoneLogin: defaultCompany.allowPhoneLogin,
          requireEmailVerification: defaultCompany.requireEmailVerification,
          requirePhoneVerification: defaultCompany.requirePhoneVerification,
        },
        ...companies
      ];
    } else {
      // Ensure DakDam MLM is at the top of the list
      const dakdamIndex = companies.findIndex(c => c.name === 'DakDam MLM');
      if (dakdamIndex > 0) {
        const dakdam = companies.splice(dakdamIndex, 1)[0];
        companies = [dakdam, ...companies];
      }
    }

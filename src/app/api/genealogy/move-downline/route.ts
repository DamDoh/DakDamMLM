...
async function executeMovement(
  ...
  // Update teamSize for both old and new parents and all their upline sponsors
  try {
    const { PVMatchingService } = await import('@/services/pv-matching-service');
    
    // Update old parent's teamSize
    await PVMatchingService.updateTeamSize(currentParent.id);
    
    // Update new parent's teamSize
    await PVMatchingService.updateTeamSize(newParent.id);
    
    // Update all upline sponsors' teamSize (cascade)
    // Get all unique upline sponsors from both parents
    const allUplineIds = new Set<string>();
    
    const oldParentUplines = await PVMatchingService.getAllUplineSponsors(currentParent.id);
    oldParentUplines.forEach(id => allUplineIds.add(id));
    
    const newParentUplines = await PVMatchingService.getAllUplineSponsors(newParent.id);
    newParentUplines.forEach(id => allUplineIds.add(id));
    
    // Also update uplines of the moved member
    const memberUplines = await PVMatchingService.getAllUplineSponsors(member.id);
    memberUplines.forEach(id => allUplineIds.add(id));
    
    await Promise.all(
      Array.from(allUplineIds).map(uplineId => PVMatchingService.updateTeamSize(uplineId))
    );
  } catch (error) {
    logger.error('Error updating team sizes after movement:', {
      error: error instanceof Error ? error.message : 'Unknown error',
      memberId: member.id
    });

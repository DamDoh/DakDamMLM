...
class DisasterRecoveryService {
  // Initialized in initializeAwsClients, asserted with definite assignment
    ...
    for (const region of [this.config.primaryRegion, ...this.config.secondaryRegions]) {
      for (const service of services) {
        try {
          ...
          logger.error(`Health check failed for ${service} in ${region}:`, { error });
          results.push({
            service,
            region,
            status: 'unhealthy',
...
Metrics: ${JSON.stringify(result.metrics, null, 2)}
    ...
    // Record in monitoring as an error event (health alerts are operational, not business metrics)
    apmMonitoring.recordError(
      new Error(`Service ${result.service} unhealthy in ${result.region}`),
      {
        service: result.service,
        region: result.region,
  ...
  async monitorGeoReplication(): Promise<GeoReplicationStatus> {
    ...
    try {
      ...
      logger.error('Geo-replication monitoring failed:', { error });
      status.dataConsistency = 'out-of-sync';
    ...
    try {
      ...
      for (const restorePoint of backup.restorePoints) {
        try {
          ...
          logger.error(`Failed to restore service ${restorePoint.service}:`, { error });
          throw error;
        }
      }
  ...
  async getAvailabilityMetrics(timeRange: 'hour' | 'day' | 'week' | 'month' = 'month'): Promise<{
    ...
    try {
      ...
      logger.error('Failed to get availability metrics:', { error });
      throw error;
    }
  }
  ...
  private async performMaintenanceWindow(): Promise<void> {
    try {
      ...
      logger.error('Maintenance window operations failed:', { error });
    }
  }
  ...
  private async rollbackFailover(failoverEvent: FailoverEvent): Promise<void> {
    try {
      ...
      logger.error(`Failover rollback failed: ${failoverEvent.id}`, { error });
    }
  }

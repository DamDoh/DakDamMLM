...
export default function OnboardingProgress() {
  ...
  const refreshOnboardingProgress = useCallback(async () => {
    if (!user) {
      console.warn('[Onboarding] Cannot refresh: user not found');
      return null;
    }

    try {
      console.log(`[Onboarding] Refreshing progress for user ${user.id}`);
      ...
      if (!res.ok) {
        ...
        console.error(`[Onboarding] Failed to fetch onboarding (${res.status}):`, errorText);
        throw new Error(`Failed to fetch onboarding: ${res.status}`);
      }

      const json = await res.json();
      console.log(`[Onboarding] Refresh response:`, json);
      
      const progress = (json?.data?.progress || null) as MemberProgress | null;
      
      if (progress) {
        console.log(`[Onboarding] ✅ Updated progress:`, {
        ...
        console.warn('[Onboarding] No progress data in response');
      }
      
      return progress;
    } catch (error: any) {
      console.error('[Onboarding] ❌ Error refreshing progress:', error);
      toast({ 
        variant: 'destructive', 
        title: t('onboarding.errorTitle'), 
        description: error.message || t('onboarding.errorRefreshProgress') 
      });
      return null;
    }
  }, [user, toast, t]);
  ...
  const handleCompleteStep = useCallback(async (step: OnboardingStep) => {
    if (!user) {
      console.warn('[Onboarding] Cannot complete step: user not found');
      return;
    }

    console.log(`[Onboarding] Attempting to complete step: ${step.id} for user ${user.id}`);
    ...
    startTransition(async () => {
        try {
          ...
          console.log(`[Onboarding] Sending request to complete step ${step.id}`);
          ...
          const res = await fetch('/api/onboarding', {
            ...
            body: JSON.stringify({ 
              ...
              stepId: step.id, 
              timeSpent: step.estimatedMinutes || 0 
            }),
          });
          ...
          if (!res.ok) {
            const errorText = await res.text().catch(() => 'Unknown error');
            console.error(`[Onboarding] API error (${res.status}):`, errorText);
            throw new Error(`API error: ${res.status} - ${errorText}`);
          }

          const json = await res.json();
          console.log(`[Onboarding] Response data:`, json);
          
          const success = !!json?.success;
          if (success) {
              console.log(`[Onboarding] ✅ Step ${step.id} completed successfully`);
              toast({
                  title: t('onboarding.stepCompletedTitle'),
                  description: t('onboarding.stepCompletedDescription', { stepTitle: step.title }),
              });
              // Refresh progress after a short delay to ensure DB is updated
              setTimeout(async () => {
                await refreshOnboardingProgress();
              }, 500);
          } else {
              const errorMsg = json?.error || 'Server returned false for completeOnboardingStep';
              console.error(`[Onboarding] ❌ Server returned false:`, errorMsg);
            ...
            toast({
                variant: 'destructive',
                title: t('onboarding.errorTitle'),
                description: error.message || t('onboarding.errorUpdateProgress'),
            });
        }
    });
  ...
  const renderStepAction = (step: OnboardingStep) => {
    switch (step.id) {
        case 'first-product-order':
            return <Button asChild size="sm" variant="outline"><Link href="/product">{t('onboarding.placeOrder')}</Link></Button>;
        case 'setup-profile':
            return <Button asChild size="sm" variant="outline"><Link href="/profile">{t('onboarding.goToProfile')}</Link></Button>;
        default:
            return (
              <Button 
                size="sm" 
                ...
                onClick={(e) => {
                  e.preventDefault();
                  e.stopPropagation();
                  console.log(`[Onboarding] Button clicked for step: ${step.id}`);
                  handleCompleteStep(step);
                }} 
                disabled={isPending}
              >
                {isPending ? t('onboarding.completing') : t('onboarding.complete')}
              </Button>
            );
    }
  }
  ...
  const getStepTitle = (step: OnboardingStep) => {
    const translationKey = `onboarding.step.${step.id}.title`;
    const translated = t(translationKey);
    // If translation exists (not the same as key), use it; otherwise fallback to step.title
    return translated !== translationKey ? translated : step.title;
  }

  const getStepDescription = (step: OnboardingStep) => {
    const translationKey = `onboarding.step.${step.id}.description`;
    const translated = t(translationKey);
    // If translation exists (not the same as key), use it; otherwise fallback to step.description
    return translated !== translationKey ? translated : step.description;
  }
            ...
            return (
              <div key={step.id} className={cn("flex items-start gap-4 p-3 rounded-lg transition-colors", isCurrent ? 'bg-primary/10' : 'bg-muted/30')}>
                ...
                <div className="flex-1">
                  <p className={cn("font-medium", isCompleted && "line-through text-muted-foreground")}>{getStepTitle(step)}</p>
                  <p className={cn("text-sm text-muted-foreground", isCompleted && "line-through")}>{getStepDescription(step)}</p>
                </div>

...
export default function GenealogyTree({ node, onNodeClick, allMembers, onAddMember, onEditMember, onMemberDetail, isLoading }: GenealogyTreeProps) {
  ...
  const renderChild = (childId: string | null, position: 'left' | 'right') => {
    ...
    // If the childId exists but member is missing (deleted), allow adding again
    if (childId) {
      const member = allMembers.get(childId);
      if (member) {
        // Check if this member can be edited (onEditMember callback handles permission check)
        return <MemberNode member={member} onNodeClick={onNodeClick} onEditClick={onEditMember} onDetailClick={onMemberDetail}/>;
      }
      // Missing member -> show add placeholder only if onAddMember is provided (admin/stockist only)
      return <MemberNode onNodeClick={() => {}} onAddClick={onAddMember ? () => onAddMember(node.id, position) : undefined} />;
    }
    
    // Otherwise, render the placeholder to add a new member only if onAddMember is provided (admin/stockist only)
    return <MemberNode onNodeClick={() => {}} onAddClick={onAddMember ? () => onAddMember(node.id, position) : undefined} />;
  };
